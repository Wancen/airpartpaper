load(file="C:/Users/wancenmu/Test Dropbox/Wancen Mu/ASEsummaries_cd4tcLS/processed.RData")
...
il10<-str_which(rownames(sce),"IL10")
ubash3a<-str_which(rownames(sce),"UBASH3A")
...
table(keep_feature)
## selecting 1065 genes
sce<-sce[keep_feature,]
save(sce,file="C:/Users/wancenmu/Test Dropbox/Wancen Mu/ASEsummaries_cd4tcLS/processed2.RData")
load(file="C:/Users/wancenmu/Test Dropbox/Wancen Mu/ASEsummaries_cd4tcLS/processed2.RData")
...
save(res,file="C:/Users/wancenmu/Test Dropbox/Wancen Mu/ASEsummaries_cd4tcLS/DAI_500.RData")
load(file="C:/Users/wancenmu/Test Dropbox/Wancen Mu/ASEsummaries_cd4tcLS/DAI_500.RData")
...
# ratio_pseudo2 <- assay(sce_cluster,"ratio_pseudo")
# ratio2 <- assay(sce_cluster,"ratio")
# ratio_pseudo_cluster <- limma::removeBatchEffect(ratio_pseudo2, batch=y)
# ratio_cluster <- limma::removeBatchEffect(ratio2, batch=y)
# ### remove row means
# ratio_pseudo_cluster <- ratio_pseudo_cluster - rowMeans(ratio_pseudo_cluster)
# ratio_cluster <- ratio_cluster - rowMeans(ratio_cluster,na.rm = T)
# assay(sce_cluster,"ratio_pseudo") <- ratio_pseudo_cluster
# assay(sce_cluster,"ratio") <- ratio_cluster
# sce_cluster <- geneCluster(sce_cluster,G=4)
# metadata(sce_cluster)$geneCluster
#
library(tidyverse)
res2 <- res[,geneVar_name] %>% t()
# res2 <- res2 - rowMeans(res2)
#
# summary_select<-summaryAllelicRatio(sce_cluster)
# for(i in 1:length(summary_select)) {
#   inst <- summary_select[[i]]
#   inst_order <- inst[order(inst$weighted.mean), ]
#   if(max(diff(inst_order$weighted.mean)) > 0.05){
#     print(i)
#   }}
...
# my_clusters <- test$cluster
my_clusters <- c(2,3,1,1,2,1,1,2,4,2,
                 1,2,3,3,3,2,1,4,3,3,
                 2,1,2,2,2,1,2,2,3,4,
                 2,2,2,1,1,4,1,1,3,4,
                 3,1,1,3,2,3,1,4,1,3,
...
patchwork1 <- g3/g4
patchwork2 <-g1/g2
patchwork <- patchwork2 | patchwork1
