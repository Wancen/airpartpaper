...
# bas <- read.table("C:/Users/wancen/Test Dropbox/Wancen Mu/ASEsummaries_cd4tcLS/summary_both_alleles_seen_swappedT4T8.txt", header = T, stringsAsFactors = F, row.names = 1)
# reference allele counts per site
ref <- read.table("C:/Users/wancen/Test Dropbox/Wancen Mu/ASEsummaries_cd4tcLS/summary_refCount_swappedT4T8.txt", header = T, stringsAsFactors = F, row.names = 1)
# alternative allele counts per site
alt <- read.table("C:/Users/wancen/Test Dropbox/Wancen Mu/ASEsummaries_cd4tcLS/summary_altCount_swappedT4T8.txt", header = T, stringsAsFactors = F, row.names = 1)
# gene snp mapping file
snp2gene <- read.table("C:/Users/wancen/Test Dropbox/Wancen Mu/ASEsummaries_cd4tcLS/gene_snp_mapping_file.txt", stringsAsFactors = F) %>% `colnames<-`(c("chr","pos","ID", "intergenic"))
...
save(geneGuess,index,snpindex,snpname,a1_new,a2_new,id,cell_meta_unique,order_of_development,file="./data/dynamicASE.rda")
