...
We provide two methods for gene clustering. Gaussian mixture modeling was used in this case and we set a relatively small number of cluster so that a larger gene set may more meaningful in GO term analysis. Hierarchical clustering is an alternative way for clustering. 
...
# jpeg(file="C:/Users/wancen/OneDrive - University of North Carolina at Chapel Hill/Lab/GRA/plot/heatmap_larsson.jpeg",width = 8, height = 6,units = "in",res=450)
...
fsr <- extractResult(sce_sub,estimates = "fsr")
lower <- extractResult(sce_sub,"lower")
upper <- extractResult(sce_sub,"upper")
## number of genes are allelic imbalance in stem cell
indexci<- which(svalue[,1]<0.005)
length(indexci)
## number of genes are allelic imbalance in fibroblast
indexci<- which(svalue[,2]<0.005)
length(indexci)
```
...
makeForest <- function(sce, genepoi, ctpoi = seq_len(nlevels(sce$x)), showtext = FALSE, xticks, boxsize = .25,
  ...
  ar <- rowData(sce)[, c(grep("ar_", colnames(rowData(sce)), value = TRUE))] %>% `colnames<-`(levels(sce$x))
  if (showtext) {
  ...
  if (missing(genepoi)) {
    ...
    genepoi <- head(order(ar$mESC,decreasing = T), min(30, nrow(sce)))
  }
  ...
  lower2 <- lower
  lower2[,2:4] <- 1 - upper[,2:4]
  upper2 <- upper
  upper2[,2:4] <- 1 - lower[,2:4]
...
```{r}
jpeg(file="C:/Users/wancen/OneDrive - University of North Carolina at Chapel Hill/Lab/GRA/plot/larssonviolin2.jpeg",width = 4.5, height = 2.5,units = "in",res=450)
makeViolin(sce_sub)
dev.off()
```

```{r, warning=FALSE, fig.width=12}
# jpeg(file="C:/Users/wancen/OneDrive - University of North Carolina at Chapel Hill/Lab/GRA/plot/forest.jpeg",width = 10, height = 6,units = "in",res=450)
...
Through GO term evaluation, we can find indeed `response to leukemia inhibitory factor` GO terms(GO:1990823) has the most significant adjust p value = 0.136.
...
head(gores[gores$ontology=="BP" & gores$numInCat>15,], 15) %>% 
  kbl() %>%
...
```{r}
contagency_table<-matrix(c(overlaptxb_prom_total,overlaptxb_otherprom_total),ncol=2,byrow = T)
print(contagency_table)
## calculate pvalue
res<-fisher.test(contagency_table,alternative = "g")
res$p.value
```

### Permutation test
Random  sampled  the  same  number  of genes in the interested gene cluster from the null cluster and the interested gene cluster and calculate odds ratio of stem cell verses fibroblast among the  sampled  genes  and  the  left  genes.  Repetitive  500  times.  Sampling distribution of the odds ratio was roughly following Gaussian distribution. And the odds ratio calculated from original data is far away from the sampling distribution, indicating that this gene cluster are significantly differ with null (p <10−4) and it is consistent with Fisher-exact test results.
```{r, eval=T}
library(pbapply)
...
p<-ggplot(or, aes(x=or)) + 
  ...
  geom_point(
    ...
    color="red",
    size=4)
# jpeg(file="C:/Users/wancen/OneDrive - University of North Carolina at Chapel Hill/Lab/GRA/plot/permutate.jpeg",width = 3, height = 2,units = "in",res=450)
p
# dev.off()
```
