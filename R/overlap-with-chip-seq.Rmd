...
# if(max(diff(summary2$weighted.mean))>0.1){
    ...
    overlaptxb<-get_overlap(gene_prom,feat,mef_filter,mESC_filter)
...
othergenes<-sample(feat2,length(feat))
overlaptxb2<-get_overlap(gene_prom,othergenes,mef_filter,mESC_filter)
...
# If x/y>x'/y'
```{r}
typeratio1<-overlap$peak_count.x/overlap$peak_count.y
typeratio2<-overlap2$peak_count.x/overlap2$peak_count.y
wilcox.test(typeratio1,typeratio2,alternative = "g")
```
