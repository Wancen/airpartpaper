...
mESC_filter<-mESC[which(mcols(mESC)$"-log(FDR)/log(10)" > 2)]
...
mef_filter<-mef[which(mcols(mef)$"-log(FDR)/log(10)" > 2)]
...
saveDb(txdb, file="gencode.vM1.sqlite")
txdb <- loadDb("gencode.vM1.sqlite")
...
# these are actually not promoters, but a wide net for enhancers
gene_prom <- promoters(g, upstream=5e4, downstream=5e4)
...
int_genes<-c(sum(overlaptxb$peak_count.x,na.rm = TRUE),sum(overlaptxb$peak_count.y,na.rm = TRUE))
```
...
n <- 18
feat2.trim <- feat2[names(feat2) %in% g$sym]
set.seed(5)
othergenes <- replicate(100, sample(feat2.trim,n,replace=FALSE), simplify=FALSE) #sample 100 times
pvals <- pbapply::pbsapply(1:100, function(i) {
  other_overlap <- get_overlap(gene_prom, names(othergenes[[i]]), mef_filter, mESC_filter)
  print(knitr::kable(other_overlap))
  other_genes_sum <- c(sum(other_overlap$peak_count.x, na.rm=TRUE),
                       sum(other_overlap$peak_count.y, na.rm=TRUE))
  chisq.test(rbind(int_genes, other_genes_sum))$p.value
})
summary(pvals)
